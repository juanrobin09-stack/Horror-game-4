...
init python:
    ...
    # Callback pour images manquantes (fallback gracieux)
    def missing_image_callback(name):
        name_str = str(name).lower()
        if "bg" in name_str:
            if "rouge" in name_str or "red" in name_str: return Solid("#1a0000")
            if "jour" in name_str or "aube" in name_str or "day" in name_str or "dawn" in name_str: return Solid("#1a1a2e")
            if "code" in name_str or "ecran" in name_str or "screen" in name_str: return Solid("#0a0a15")
            if "hopital" in name_str or "hospital" in name_str: return Solid("#e8e8e8")
            if "bsod" in name_str: return Solid("#0078d7")
            if "glitch" in name_str: return Solid("#0d0d1a")
            return Solid("#0a0a12")
        if "overlay" in name_str: return Solid("#00000000")
        if "jumpscare" in name_str: return Solid("#ff000088")
        if any(x in name_str for x in ["entite","kane","marc","sarah","alex"]): return Null()
        return Solid("#1a1a2e88")
...
# === MUSIQUES D'AMBIANCE ===
# Note : menu_theme.ogg, chase.ogg et silence.ogg n'existent pas dans le dossier
# Utilisation de fichiers existants comme alternatives
define audio.menu_theme = "audio/drone_grave.ogg"             # Menu principal (drone sombre) - Alternative
...
define audio.chase = "audio/tension_forte.ogg"                # Poursuite/urgence - Alternative
define audio.drone_grave = "audio/drone_grave.ogg"            # Drone basse fréquence (malaise)
define audio.silence = "audio/ambiance_nuit.ogg"              # Silence oppressant - Alternative (volume réduit)
...
# === EFFETS SONORES ===
# Note : Les fichiers sont dans game/audio/, pas dans game/audio/sfx/
define audio.notification = "audio/notification.ogg"      # Bip de notification
define audio.error = "audio/error.ogg"                    # Son d'erreur Windows
define audio.startup = "audio/startup.ogg"                # Démarrage PC
define audio.shutdown = "audio/shutdown.ogg"              # Extinction PC
define audio.glitch = "audio/glitch.ogg"                  # Glitch électronique
define audio.glitch_long = "audio/glitch_long.ogg"        # Glitch prolongé
define audio.static = "audio/static.ogg"                  # Bruit statique TV
define audio.heartbeat = "audio/heartbeat.ogg"            # Battement de coeur
define audio.heartbeat_fast = "audio/heartbeat_fast.ogg"  # Coeur rapide (panique)
define audio.breath = "audio/breath.ogg"                  # Respiration lourde
define audio.breath_close = "audio/breath_close.ogg"      # Respiration proche (terrifiant)
define audio.typing = "audio/typing.ogg"                  # Frappe clavier
define audio.typing_slow = "audio/typing_slow.ogg"        # Frappe lente (entité)
define audio.phone_ring = "audio/phone_ring.ogg"          # Téléphone sonne
define audio.phone_vibrate = "audio/phone_vibrate.ogg"    # Téléphone vibre
define audio.door_knock = "audio/door_knock.ogg"          # Coup à la porte
define audio.door_slam = "audio/door_slam.ogg"            # Porte claquée
define audio.glass_break = "audio/glass_break.ogg"        # Verre brisé
define audio.light_flicker = "audio/light_flicker.ogg"    # Lumière qui grésille
define audio.lights_off = "audio/lights_off.ogg"          # Lumières qui s'éteignent
define audio.lights_on = "audio/lights_on.ogg"            # Lumières qui s'allument
define audio.tv_on = "audio/tv_on.ogg"                    # TV qui s'allume
define audio.scream = "audio/scream.ogg"                  # Cri distant
define audio.whisper = "audio/whisper.ogg"                # Chuchotement
define audio.laugh = "audio/laugh.ogg"                    # Rire sinistre
define audio.jumpscare = "audio/jumpscare.ogg"            # Son de jumpscare
define audio.bass_drop = "audio/bass_drop.ogg"            # Impact grave
define audio.reverse = "audio/reverse.ogg"                # Son inversé (creepy)
define audio.message_send = "audio/message_send.ogg"      # Envoi message
define audio.message_receive = "audio/message_receive.ogg" # Réception message
define audio.webcam_click = "audio/webcam_click.ogg"      # Clic webcam (photo)
define audio.file_delete = "audio/file_delete.ogg"        # Suppression fichier
define audio.access_denied = "audio/access_denied.ogg"    # Accès refusé
define audio.success = "audio/success.ogg"                # Succès/validation
# Note : clock_tick.ogg n'existe pas, utilisation de countdown.ogg comme alternative
define audio.clock_tick = "audio/countdown.ogg"          # Tic-tac horloge - Alternative
define audio.countdown = "audio/countdown.ogg"            # Compte à rebours
define audio.electricity = "audio/electricity.ogg"        # Grésillement électrique
define audio.upload = "audio/upload.ogg"                  # Son d'upload en cours
...
image kane triste = "images/kane_triste.png"                # Kane triste (vulnérabilité)
...
init python:
    ...
    # Fonction safe_play pour éviter les crashes audio
    def safe_play(audio_file, channel="sound", volume=1.0):
        try:
            if channel == "music":
                renpy.music.play(audio_file, channel="music", fadein=1.0, volume=volume)
            else:
                renpy.sound.play(audio_file, volume=volume)
        except:
            pass
    
    # Jouer un son avec volume (fonction legacy, utilise safe_play en interne)
    def play_sfx(sound, volume=1.0):
        safe_play(sound, channel="sound", volume=volume)
    ...
    # Système d'aide pour les énigmes
    def demande_aide_enigme(numero_enigme):
        """Affiche un indice pour l'énigme, avec pénalité de santé mentale"""
        if language == "fr":
            if numero_enigme == 1:
                renpy.notify("💡 Indice : Pense à la liberté... en anglais... à l'envers...")
            elif numero_enigme == 2:
                renpy.notify("💡 Indice : L'ordre des mots dans une phrase normale...")
            elif numero_enigme == 3:
                renpy.notify("💡 Indice : Cherche l'adresse qui n'a pas de nom... et un chiffre spécial...")
            elif numero_enigme == 4:
                renpy.notify("💡 Indice : Les initiales des victimes... et le numéro du dossier...")
            elif numero_enigme == 5:
                renpy.notify("💡 Indice : Quelle fonction cible spécifiquement Kane ?")
            elif numero_enigme == 7:
                renpy.notify("💡 Indice : Le chemin bleu révèle la vérité...")
            elif numero_enigme == 8:
                renpy.notify("💡 Indice : Le code que tu as découvert plus tôt...")
        else:
            if numero_enigme == 1:
                renpy.notify("💡 Hint: Think of freedom... in English... backwards...")
            elif numero_enigme == 2:
                renpy.notify("💡 Hint: The order of words in a normal sentence...")
            elif numero_enigme == 3:
                renpy.notify("💡 Hint: Look for the address with no name... and a special number...")
            elif numero_enigme == 4:
                renpy.notify("💡 Hint: The victims' initials... and the folder number...")
            elif numero_enigme == 5:
                renpy.notify("💡 Hint: Which function specifically targets Kane?")
            elif numero_enigme == 7:
                renpy.notify("💡 Hint: The blue path reveals the truth...")
            elif numero_enigme == 8:
                renpy.notify("💡 Hint: The code you discovered earlier...")
        # Pénalité pour demander de l'aide
        store.sante_mentale = max(0, store.sante_mentale - 3)
    
    # Sauvegarde automatique avant énigme
    def sauvegarde_avant_enigme(nom_enigme):
        """Crée une sauvegarde automatique avant une énigme importante"""
        try:
            renpy.save("enigme_" + str(nom_enigme), extra_info="Avant énigme " + str(nom_enigme))
        except:
            pass
    
    # Permettre de rejouer une énigme échouée (avec pénalité)
    def peut_rejouer_enigme(numero_enigme):
        """Vérifie si le joueur peut rejouer une énigme échouée"""
        if not hasattr(store, 'enigmes_rejouees'):
            store.enigmes_rejouees = {}
        if numero_enigme not in store.enigmes_rejouees:
            store.enigmes_rejouees[numero_enigme] = 0
        return store.enigmes_rejouees[numero_enigme] < 1  # 1 seule reprise autorisée
    
    def rejouer_enigme(numero_enigme):
        """Marque qu'une énigme est rejouée et applique une pénalité"""
        if not hasattr(store, 'enigmes_rejouees'):
            store.enigmes_rejouees = {}
        store.enigmes_rejouees[numero_enigme] = store.enigmes_rejouees.get(numero_enigme, 0) + 1
        store.sante_mentale = max(0, store.sante_mentale - 10)  # Pénalité importante
        if language == "fr":
            renpy.notify("⚠ Réessai autorisé... mais à quel prix ?")
        else:
            renpy.notify("⚠ Retry allowed... but at what cost?")
